...
if (isset($_POST['delete-mbarang'])) {
  $cekdata = mysqli_query($conn, "SELECT * FROM mbarang WHERE id_mbarang = '$_POST[id_mbarang]'");
  if (mysqli_num_rows($cekdata) > 0) {
    $ada = mysqli_fetch_assoc($cekdata);
    if (deleteFile($ada['foto_mbarang'], $backurl . 'uploads/model-barang') && setDelete('mbarang', ['id_mbarang' => $_POST['id_mbarang']])) {
...
<body class="sidebar-mini layout-fixed control-sidebar-slide-open text-sm">
  <div class="wrapper">
                    ...
                    <ul class="list-group list-group-unbordered">
                      ...
                      <li class="list-group-item pb-0" style="border-bottom: 0;">
                        <div class="input-group">
                          <div class="row w-100 ml-0 mr-0">
                            <div class="col-md-12">
                              <label class="float-right" for="foto_mbarang">Foto Barang</label>
                              <div class="input-group">
    ...
    <div class="modal fade" id="delete-mbarang">
      <form method="POST" class="modal-dialog" enctype="multipart/form-data" autocomplete="off">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">Hapus mbarang</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <h4 class="modal-title">Anda yakin Ingin Menghapus mbarang ini?</h4>
            <input type="hidden" name="id_mbarang" id="did_mbarang">
          </div>
          <div class="modal-footer">
            <div class="btn-group btn-block">
              <button class="btn btn-outline-success" data-dismiss="modal">Batal</button>
              <button type="submit" name="delete-mbarang" class="btn btn-outline-danger">Hapus</button>
            </div>
          </div>
        </div>
        <!-- /.modal-content -->
      ...
      var table_mbarang = $('#table_mbarang').DataTable({
        ...
        "columns": [{
          ...
          "render": function(data, type, row, meta) {
            return '<div class="btn-group"><button type="button" class="btn btn-sm btn-danger" data-toggle="modal" onclick="$(\'#did_mbarang\').val(\'' + data + '\')" data-target="#delete-mbarang"><i class="fa fa-trash-alt"></i></button><a href="' + host + 'model-barang/' + data + '" class="btn btn-sm bg-info"><i class="fa fa-edit"></i></a></div>';
          }
        }],
      });
